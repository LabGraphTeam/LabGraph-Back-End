...
public class CustomGlobalErrorHandling {
	...
	// ==========================================
	@ApiResponses(value = {@ApiResponse(responseCode = "400", description = "Invalid input supplied",
	...
	public ResponseEntity<ApiError> handleValidationExceptions(MethodArgumentNotValidException ex,
		...
		ApiError apiError = ApiError.of(HttpStatus.BAD_REQUEST, "Validation failed", request.getRequestURI())
	...
	public ResponseEntity<ApiError> handleMissingParameter(MissingServletRequestParameterException ex,
			HttpServletRequest request) {
		ApiError apiError = ApiError.of(HttpStatus.BAD_REQUEST, "Missing required parameter", request.getRequestURI());
		apiError.details().add(
				"Required parameter '" + ex.getParameterName() + "' of type " + ex.getParameterType() + " is missing");
		apiError.details().add("Please provide all required parameters and try again.");

		log.error("400 Bad Request: Missing required parameter [{}]: {}", request.getRequestURI(),
				ex.getParameterName());
		return ResponseEntity.badRequest().body(apiError);
	}
	...
	public ResponseEntity<ApiError> handleConstraintViolation(ConstraintViolationException ex,
		...
		ApiError apiError =
				ApiError.of(HttpStatus.BAD_REQUEST, "Validation failed", request.getRequestURI()).withDetails(details);
	...
	public ResponseEntity<ApiError> handleAuthenticationErrors(Exception ex, HttpServletRequest request) {
		...
		ApiError apiError =
				ApiError.of(HttpStatus.UNAUTHORIZED, errorMessage, request.getRequestURI()).withDetails(details);

		log.error("401 Unauthorized: Auth failed [{}] - {}", request.getRequestURI(), ex.getClass().getSimpleName());
	...
	// ==========================================
	@ApiResponses(value = {@ApiResponse(responseCode = "403", description = "Invalid recovery token",
			content = @Content(schema = @Schema(implementation = ApiError.class)))})
	...
	public ResponseEntity<ApiError> handleRecoveryTokenInvalid(RecoveryTokenInvalidException ex,
			HttpServletRequest request) {
		ApiError apiError =
				ApiError.of(HttpStatus.FORBIDDEN, "Recovery token is invalid or expired", request.getRequestURI());
	...
	public ResponseEntity<ApiError> handleAccessDenied(AccessDeniedException ex, HttpServletRequest request) {
		ApiError apiError = ApiError.of(HttpStatus.FORBIDDEN, "Access denied", request.getRequestURI());
	...
	public ResponseEntity<ApiError> handleUserNotFound(UserNotFoundException ex, HttpServletRequest request) {
		ApiError apiError =
				ApiError.of(HttpStatus.NOT_FOUND, "User not found or invalid credentials", request.getRequestURI());
	...
	@ResponseStatus(HttpStatus.NOT_FOUND)
	public ResponseEntity<ApiError> handleEndpointNotFound(NoHandlerFoundException ex, HttpServletRequest request) {
		ApiError apiError =
				new ApiError(LocalDateTime.now(), HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(),
						"The requested endpoint was not found", request.getRequestURI(), new ArrayList<>());
		...
		apiError.details().add("If you believe this is a mistake, please contact our support team.");
	...
	public ResponseEntity<ApiError> handleMethodNotAllowed(HttpRequestMethodNotSupportedException ex,
			HttpServletRequest request) {
		ApiError apiError = ApiError.of(HttpStatus.METHOD_NOT_ALLOWED, "Method Not Allowed", request.getRequestURI());
		apiError.details().add("The HTTP method used is not supported for this endpoint.");
		apiError.details()
				.add("Allowed methods: " + String.join(", ", Objects.requireNonNull(ex.getSupportedMethods())));
		apiError.details().add("Please check the API documentation for the correct HTTP method to use.");
	...
	public ResponseEntity<ApiError> handleUserAlreadyExist(UserAlreadyExistException ex, HttpServletRequest request) {
		ApiError apiError = ApiError.of(HttpStatus.CONFLICT, "A user with this username or email already exists",
	...
	@ResponseStatus(HttpStatus.CONFLICT)
	public ResponseEntity<ApiError> handleDuplicateKeyException(DuplicateKeyException ex, HttpServletRequest request) {
		ApiError apiError = ApiError.of(HttpStatus.CONFLICT, "Duplicate record exists", request.getRequestURI());
	...
	public ResponseEntity<ApiError> handleEmailSendingError(EmailSendingException ex, HttpServletRequest request) {
		ApiError apiError =
				ApiError.of(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to send email", request.getRequestURI());
		apiError.details().add("The system was unable to send the email.");
		apiError.details().add("Please try again later or contact support if the problem persists, sorry for that.");

		String cause = ex.getCause() != null ? ex.getCause().getMessage() : "Unknown";
		log.error("500 Server Error: Email failed [{}] - Cause: {}", request.getRequestURI(), cause);
	...
	public ResponseEntity<ApiError> handleAllUncaughtException(Exception ex, HttpServletRequest request) {
		...
		apiError.details().add("The system encountered an unexpected error while processing your request.");
		apiError.details()
				.add("Our technical team has been notified and is working to resolve this issue, sorry for that :(.");
		apiError.details().add("Please try again later or contact support if the problem persists.");
	...
	public ResponseEntity<ApiError> handleBadSqlGrammarException(BadSqlGrammarException ex,
			HttpServletRequest request) {
		ApiError apiError =
				ApiError.of(HttpStatus.INTERNAL_SERVER_ERROR, "Database query error", request.getRequestURI());
	...
	@ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
	public ResponseEntity<ApiError> handleQueryTimeoutException(QueryTimeoutException ex, HttpServletRequest request) {
		ApiError apiError =
				ApiError.of(HttpStatus.INTERNAL_SERVER_ERROR, "Database operation timed out", request.getRequestURI());
	...
	public ResponseEntity<ApiError> handleDataRetrievalFailure(DataRetrievalFailureException ex,
			HttpServletRequest request) {
		ApiError apiError =
				ApiError.of(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to retrieve data", request.getRequestURI());
	...
	public ResponseEntity<ApiError> handleDataAccessException(DataAccessException ex, HttpServletRequest request) {
		...
		apiError.details().add("Our technical team has been notified and is working to resolve this issue.");
	...
	public ResponseEntity<ApiError> handleSQLException(SQLException ex, HttpServletRequest request) {
		ApiError apiError = ApiError.of(HttpStatus.INTERNAL_SERVER_ERROR, "Database error", request.getRequestURI());
		apiError.details().add("A database error occurred while processing your request.");
		apiError.details().add("Our technical team has been notified and is working to resolve this issue.");

		log.error("500 Server Error: SQL exception [{}] - State: {}, Code: {}", request.getRequestURI(),
	...
	public ResponseEntity<ApiError> handleDatabaseConnectionException(DatabaseConnectionException ex,
			HttpServletRequest request) {
		ApiError apiError =
				ApiError.of(HttpStatus.SERVICE_UNAVAILABLE, "Database connection error", request.getRequestURI());
		...
		log.error("503 Service Unavailable: Database connection error [{}]", request.getRequestURI(), ex);
